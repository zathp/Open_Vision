...
class ColorReplacerBatchApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Color Replacer - Batch Mode")
        
        # Store multiple images
        self.images = []  # List of dicts: {'path': str, 'original': Image, 'modified': Image, 'name': str}
        ...
        # Load buttons
        tk.Button(self.left_frame, text="Load Images", command=self.load_images).pack(pady=5)
        
        # Image list
        tk.Label(self.left_frame, text="Loaded Images:").pack()
        self.image_listbox = tk.Listbox(self.left_frame, width=30, height=8)
        self.image_listbox.pack(pady=5)
        self.image_listbox.bind('<<ListboxSelect>>', self.on_image_select)
        
        # Original colors list
        tk.Label(self.left_frame, text="Original Colors:").pack()
        self.original_listbox = tk.Listbox(self.left_frame, width=30, height=15)
        ...
        self.replacement_listbox = tk.Listbox(self.left_frame, width=30, height=15)
        ...
        # Apply button
        apply_btn_frame = tk.Frame(self.left_frame)
        apply_btn_frame.pack(pady=5)
        tk.Button(apply_btn_frame, text="Apply to Current", command=self.apply_to_current).pack(side=tk.LEFT, padx=2)
        tk.Button(apply_btn_frame, text="Apply to All Images", command=self.apply_to_all).pack(side=tk.LEFT, padx=2)
        
        # Save buttons
        save_btn_frame = tk.Frame(self.left_frame)
        save_btn_frame.pack(pady=5)
        tk.Button(save_btn_frame, text="Save Current", command=self.save_current).pack(side=tk.LEFT, padx=2)
        tk.Button(save_btn_frame, text="Save All", command=self.save_all).pack(side=tk.LEFT, padx=2)
        ...
        self.original_canvas.bind('<Motion>', lambda e: self.on_mouse_move(e, 'original'))
        self.modified_canvas.bind('<Motion>', lambda e: self.on_mouse_move(e, 'modified'))
        
        # Bind click events
        self.original_canvas.bind('<Button-1>', lambda e: self.on_canvas_click(e, 'original'))
        self.modified_canvas.bind('<Button-1>', lambda e: self.on_canvas_click(e, 'modified'))
    ...
    def on_image_select(self, event):
        ...
        self.display_image(current['modified'], self.modified_canvas)
        
        # Extract unique colors from this image
        img_array = np.array(current['original'])
        ...
        for color in self.unique_colors:
            ...
            # Use existing mapping if available, otherwise map to itself
            if color_tuple not in self.color_mappings:
                self.color_mappings[color_tuple] = color_tuple
            
            mapped_color = self.color_mappings[color_tuple]
            color_hex = '#{:02x}{:02x}{:02x}'.format(*color[:3])
            mapped_hex = '#{:02x}{:02x}{:02x}'.format(*mapped_color[:3])
            
            self.original_listbox.insert(tk.END, f"RGBA{color_tuple} - {color_hex}")
            self.replacement_listbox.insert(tk.END, f"RGBA{mapped_color} - {mapped_hex}")
    ...
    def apply_color_mappings(self, original_image):
        """Apply current color mappings to an image and return the result."""
    ...
    def apply_to_current(self):
        ...
        current = self.images[self.current_image_index]
        current['modified'] = self.apply_color_mappings(current['original'])
        self.display_image(current['modified'], self.modified_canvas)
        print(f"Applied changes to {current['name']}")
    ...
    def apply_to_all(self):
        ...
        for img_data in self.images:
            img_data['modified'] = self.apply_color_mappings(img_data['original'])
        
        # Refresh display if current image exists
        if self.current_image_index is not None:
            current = self.images[self.current_image_index]
            self.display_image(current['modified'], self.modified_canvas)
        
        print(f"Applied changes to all {len(self.images)} images")
    ...
    def on_mouse_move(self, event, canvas_type):
        """Display the color under the mouse cursor."""
        if self.current_image_index is None:
            return
        
        current = self.images[self.current_image_index]
        image = current['original'] if canvas_type == 'original' else current['modified']
        label = self.original_color_label if canvas_type == 'original' else self.modified_color_label
    ...
    def on_canvas_click(self, event, canvas_type):
        ...
        if 0 <= img_x < image.width and 0 <= img_y < image.height:
            ...
            pixel = current['original'].getpixel((img_x, img_y))
    ...
    def save_current(self):
        ...
        if filepath:
            current['modified'].save(filepath)
            print(f"Saved {current['name']} to {filepath}")
    ...
    def save_all(self):
        ...
        for img_data in self.images:
            ...
            ext = os.path.splitext(img_data['name'])[1]
            output_name = f"{base_name}_modified{ext}"
            output_path = os.path.join(directory, output_name)
            
            # Save the image
            img_data['modified'].save(output_path)
            print(f"Saved {output_name}")
        
        print(f"All {len(self.images)} images saved to {directory}")

if __name__ == "__main__":
    root = tk.Tk()
    app = ColorReplacerBatchApp(root)
    root.mainloop()
